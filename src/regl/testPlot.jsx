...
const ReglScatterPlot = () => {
  ...
  useEffect(() => {
    const loadCSVData = async () => {
      const data = await d3.csv("././data_coba_plot/pk-a1_results-coba.csv", (d) => ({
        x: +d.TVDSS, // Convert string to number
        y: +d.RHOB,
        color: [Math.random(), Math.random(), Math.random()], // Assign random colors
      }));
      setPoints(data);
    };
