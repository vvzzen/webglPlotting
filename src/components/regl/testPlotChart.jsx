...
import vert2 from "./glslShaders/testPlotChartLineVert.glsl?raw";
...
const ReglScatterPlot = () => {
  ...
  useEffect(() => {
    ...
    // 🔥 Create Camera only after Regl exists
    if (!cameraRef.current) {
      cameraRef.current = createCamera(reglRef.current, {
        damping: 0.01, // Smooth movement
        zoomSpeed: 0.5,
        center: [0, 0, 0],
        distance: 2,
      });
  ...
  useEffect(() => {
    ...
    const drawLineCmd = drawLine(regl, points, width, height, frag, vert2);

    // Start animation loop
    const frameLoop = regl.frame(() => {
      cameraRef.current(({ projection, view }) => {
        // 🔥 Camera automatically manages view matrix
